...
def local_action_probabilities(p_transition, terminal, reward, beta):
    """
    ...
    Args:
        ...
        beta: The confidence in the given demonstrator
            `[Integer]`
...
def compute_expected_svf(p_transition, p_initial, terminal, reward, beta, eps=1e-5):
    """
    ...
    Returns:
        ...
        `[state: Integer] -> svf: Float`.
    """
    p_action = local_action_probabilities(p_transition, terminal, reward, beta)
    return expected_svf_from_policy(p_transition, p_initial, terminal, p_action, eps)

def IRL(p_transition, features, terminal, mix_e_features, mix_p_initial, n_trajs, optim, init, betas, eps=1e-4, eps_svf=1e-5):
    """
    ...
    while delta > eps:
        ...
        for p_initial, e_features, m, beta in zip(mix_p_initial, mix_e_features, n_trajs, betas):
            e_svf = compute_expected_svf(p_transition, p_initial, terminal, reward, beta, eps = eps_svf)
            div = e_features - features.T.dot(e_svf)
            # compute gradient of theta as weighted divergence
            grad += beta*(m/sum(n_trajs))*div
...
def local_causal_action_probabilities(p_transition, 
                                      terminal, 
                                      reward, 
                                      discount, 
                                      eps=1e-5):
...
def compute_expected_causal_svf(p_transition, 
                                p_initial, 
                                terminal, 
                                reward, 
                                discount,
