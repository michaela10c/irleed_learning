...
def generate_trajectory(world, policy, start, final, horizon):
    """
    ...
    trajectory = []
    while (state not in final) and (len(trajectory)<horizon):
        action = policy(state)
...
def generate_trajectories(n, world, policy, start, final, horizon):
    """
    ...
    def _generate_one():
        ...
        return generate_trajectory(world, policy, s, final, horizon)
